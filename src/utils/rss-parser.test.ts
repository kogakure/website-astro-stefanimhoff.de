...
describe('rss-parser', () => {
	describe('stripMarkdown', () => {
		...
		it('should handle mixed formatting', () => {
			expect(stripMarkdown('This is **bold** and [a link](url)')).toBe(
	...
	describe('makeAbsolute', () => {
		...
		it('should convert relative URLs starting with /', () => {
			expect(makeAbsolute('/path/to/file', SITE_URL)).toBe(
				'https://www.stefanimhoff.de/path/to/file'
			);
			expect(makeAbsolute('/images/test.jpg', SITE_URL)).toBe(
				'https://www.stefanimhoff.de/images/test.jpg'
			);
		});
	...
	describe('fixImagePaths', () => {
		it('should make relative image src absolute', () => {
			...
			expect(result).toBe(
				'<img src="https://www.stefanimhoff.de/images/test.jpg" alt="Test" />'
			);
		});
	...
	describe('replaceImageComponent', () => {
		...
		it('should add source with link if sourceUrl provided', () => {
			const attrs =
				'src="/test.jpg" alt="Test" source="Author" sourceUrl="https://example.com"';
			const result = replaceImageComponent(attrs, SITE_URL);
	...
	describe('replaceAmazonBookComponent', () => {
		it('should create Amazon book link with image', () => {
			...
			expect(result).toContain(
				'https://images-na.ssl-images-amazon.com/images/P/B001234567.01.LZZZZZZZ.jpg'
			);
	...
	describe('replaceBlockquoteComponent', () => {
		...
		it('should convert relative sourceUrl to absolute', () => {
			...
			expect(result).toContain(
				'<cite><a href="https://www.stefanimhoff.de/blog/post">Article</a></cite>'
			);
		});
	...
	describe('stripMDXComponents', () => {
		...
		it('should replace ColorSwatchPrimary component', () => {
			const text = '<ColorSwatchPrimary color="#FF5733" name="Test" kanji="紅" />';
			const result = stripMDXComponents(text, SITE_URL);

			expect(result).toContain('background-color: #FF5733;');
		});

		it('should replace ColorSwatchMini component', () => {
			const text = '<ColorSwatchMini color="#FF5733" />';
			const result = stripMDXComponents(text, SITE_URL);
		...
		it('should handle multiple components in sequence', () => {
			const text =
				'<Image src="/img.jpg" alt="Img" /><Flag label="New" /><Ruby base="日本" text="にほん" />';
			const result = stripMDXComponents(text, SITE_URL);
